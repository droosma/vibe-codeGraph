...
| `--skip-build` | Skips the `dotnet build` step entirely |
| `--changed-only` | Only re-indexes projects with source changes since the last index |
| `--verbose` | Enables verbose progress output to stdout |
