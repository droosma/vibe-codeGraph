#!/bin/bash
# CodeGraph Query Wrapper for Claude
# Detects codegraph installation and graph location, runs query
# Outputs in a format optimized for Claude tool use
...
# Default to context format for Claude tool use
if [[ ! " $* " =~ " --format " ]]; then
    codegraph query --graph-dir "$GRAPH_DIR" --format context "$@"
else
